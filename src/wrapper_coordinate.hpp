...
vec3d gravity(vec3d pos) {
    return gravityECI(pos);
}

vec4d quat_nedg2body(vec4d quat_eci2body, vec3d pos_eci, double t) {
    Eigen::Quaterniond q(quat_eci2body[0], quat_eci2body[1], quat_eci2body[2], quat_eci2body[3]);
...
Eigen::Matrix<double, 6, 1> orbital_elements(vec3d pos_eci, vec3d vel_eci) {
    Eigen::Matrix<double, 6, 1> elem = Coordinate::orbital_elements(pos_eci, vel_eci);
